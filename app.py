...
if run_button:
    ...
    # Transmission Phase: Table + Stacked Bar Chart
    ...
    with col_trans_chart:
        # Calculate rounded maximum for y-axis
        total = results['initial_qubits']
        
        if total <= 1000:
            y_max = int(np.ceil(total / 200) * 200)
            tick_interval = 200
        elif total <= 5000:
            y_max = int(np.ceil(total / 1000) * 1000)
            tick_interval = 1000
        else:
            y_max = int(np.ceil(total / 2000) * 2000)
            tick_interval = 2000
        
        # Stacked bar chart for lost qubits vs received
        fig_trans = go.Figure()
        
        # Add received qubits (green - bottom)
        fig_trans.add_trace(go.Bar(
            x=['Channel'],
            y=[results['received_qubits']],
            name='Successfully Received',
            marker_color='#10B981',
            text=[f"{results['received_qubits']} qubits"],
            textposition='inside',
            textfont=dict(size=12, color='white', family='Arial'),
            hovertemplate='Received: %{y}<extra></extra>'
        ))
        
        # Add lost qubits (red - top)
        fig_trans.add_trace(go.Bar(
            x=['Channel'],
            y=[results['lost_qubits']],
            name='Lost in Channel',
            marker_color='#EF4444',
            text=[f"{results['lost_qubits']} qubits"],
            textposition='inside',
            textfont=dict(size=12, color='white', family='Arial'),
        ...
        fig_trans.update_layout(
            ...
            margin=dict(l=50, r=20, t=0, b=20),
            height=250
        )
        
        st.plotly_chart(fig_trans, use_container_width=True, key="transmission_stack")
    
# Key Generation Phase with funnel chart
    st.markdown("**Key Generation Phase**")
    
    col_keygen_table, col_keygen_funnel = st.columns([1.2, 1])
    
    with col_keygen_table:
    ...
    with col_keygen_funnel:
        # Create funnel chart using plotly express
        import plotly.express as px
        
        # Create DataFrame for funnel
        stages = [
            "Sifted Key",
            "After Sampling",
            "Final Secure Key"
        ...
        values = [
            results['sifted_key_length'],
            results['final_key_length'] + results['sampled_bits'],  # Before final removal
            results['final_key_length']
        ]
        
        # Create two categories to show errors
        df_clean = pd.DataFrame({
            'number': [
                results['sifted_key_length'],
                results['sampled_bits'] - results['error_count'],  # Clean sampled bits
                results['final_key_length']
            ],
            'stage': stages,
            'type': 'Valid Bits'
        ...
        df_errors = pd.DataFrame({
            'number': [
                0,
                results['error_count'],  # Errors in sampled bits
                0
            ],
            'stage': stages,
        ...
        df_funnel = pd.concat([df_clean, df_errors], axis=0)
        ...
        fig_funnel = px.funnel(
            ...
            color='type',
            color_discrete_map={
                'Valid Bits': '#10B981',  # Green
                'Errors Detected': '#EF4444'  # Red
            }
        )
        ...
        fig_funnel.update_layout(
            ...
            xaxis=dict(
                ...
                titlefont=dict(size=11, family='Georgia'),
            ...
            legend=dict(
                ...
                xanchor="center",
                x=0.5,
                font=dict(size=10, family='Georgia')
            )
        )
        ...
        st.plotly_chart(fig_funnel, use_container_width=True, key="keygen_funnel")
        
        if results['error_count'] > 0:
            st.caption(f"Red section shows {results['error_count']} errors detected in sampled bits")
